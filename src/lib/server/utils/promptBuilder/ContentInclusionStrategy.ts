 ...
 */
export class DefaultContentInclusionStrategy
	implements ContentInclusionStrategy
{
	initializeState(config: ContentInclusionConfig): ContentInclusionState {
	 ...
	 * Normalize weights so the highest weight is 1.0 while maintaining relative proportions
	 */
	protected normalizeWeights(
		weights: ContentInclusionConfig["weights"]
	): ContentInclusionConfig["weights"] {
		const weightValues = [
	...
	async processIteration(
		...
		throw new Error(
			"processIteration should be implemented by PromptBuilder"
		)
	}
	...
	shouldResetIterators(
		...
		const hasCharacterLoreAtPriority =
			priorities.characterLore.includes(priority)
		...
		return (
			(hasMessagesAtPriority ||
				hasWorldLoreAtPriority ||
				hasCharacterLoreAtPriority ||
				hasHistoryAtPriority) &&
...
export class WeightedContentInclusionStrategy extends DefaultContentInclusionStrategy {
	shouldProcessContent(
		...
		const tokenUsageRatio =
			state.totalTokens > 0
				? state.totalTokens / (state.totalTokens + 1000)
				: 0
