...
export function populateLorebookEntryBindings(
  ...
  const lorebook = chat.lorebook && chat.lorebook.id === entry.lorebookId ? chat.lorebook : undefined;
...
export function attachCharacterLoreToCharacters(
  ...
  includedCharacterLoreEntries.forEach((entry) => {
    const lorebook = chat.lorebook && chat.lorebook.id === entry.lorebookId ? chat.lorebook : undefined;
    if (!lorebook) return;
    const binding = lorebook.lorebookBindings.find((b: SelectLorebookBinding) => b.id === entry.lorebookBindingId);
  ...
  return characters.map((char) => {
    const chatChar = (chat.chatCharacters || []).find(
      (cc) => cc.character.nickname === char.nickname || cc.character.name === char.name
    );
    const charId = chatChar?.character?.id;
    return {
      ...char,
      "extra lore": charId && loreMap[charId] ? loreMap[charId] : undefined
    };
  });
}
...
export function attachCharacterLoreToPersonas(
  ...
  return personas.map((persona) => {
    ...
    return {
      ...persona,
      "extra lore": personaId && loreMap[personaId] ? loreMap[personaId] : undefined
    };
  });
}
