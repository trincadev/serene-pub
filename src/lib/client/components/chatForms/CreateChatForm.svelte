...
{#if characters.length === 0}
    ...
    <div class="flex gap-2">
        <button class="btn preset-filled-surface-500 w-full" onclick={() => (isCreating = false)}>Cancel</button>
        <button class="btn preset-filled-primary-500 w-full" onclick={goToCharactersTab}>
            Create Character
        </button>
    </div>
{:else if personas.length === 0}
    <div class="p-4 text-center text-gray-500">No personas available. Please create one first.</div>
    <div class="flex gap-2">
        <button class="btn preset-filled-surface-500 w-full" onclick={() => (isCreating = false)}>Cancel</button>
        <button class="btn preset-filled-primary-500 w-full" onclick={goToPersonasTab}>
            Create Persona
        </button>
    </div>
{:else}
    <div class="flex flex-col gap-4 p-4">
        ...
        <div>
            ...
            <select id="personaSelect" class="input input-sm w-full" bind:value={selectedPersonaId}>
                {#each personas as p}
        ...
        <div class="flex gap-2 mt-2">
            ...
            <button
                ...
                onclick={handleSave}
                disabled={!name.trim() || !selectedCharacterId || !selectedPersonaId}
            >
                Create Chat
            </button>
        </div>
