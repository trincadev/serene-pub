...
async function importDrizzleConfig() {
	try {
		const configPath = path.join(projectRoot, 'src/lib/server/db/drizzle.config.ts')
			...
			child.on('close', (code) => {
				if (code === 0) {
					try {
						...
						reject(new Error(`Failed to parse config output: ${error.message}`))
					}
				} else {
					reject(new Error(`Failed to load drizzle config, exit code: ${code}`))
				}
			})
		})
	} catch (error) {
...
async function checkDatabaseLock() {
	try {
		...
		if (currentTime < lockExpiry) {
			...
			console.error(`Database is currently locked. Lock expires in ${remainingTime} seconds.`)
			console.error('Please wait for the lock to expire or stop the running application.')
			process.exit(1)
		} else {
			// Lock is stale
